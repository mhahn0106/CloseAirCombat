...
def create_multiagent_xml(
    ...
    world_actuator = None
    world_tendons = None
    num_agents = len(all_agent_xmls)
    if rgb is None:
        rgb = get_distinct_colors(num_agents)
    RGB_tuples = list(
        map(lambda x: tuple_to_str(x), rgb)
    )
    if agent_scopes is None:
        agent_scopes = ['agent' + str(i) for i in range(num_agents)]

    if ini_pos is None:
        ini_pos = [(-i, 0, 0.75) for i in np.linspace(-num_agents, num_agents, num_agents)]
